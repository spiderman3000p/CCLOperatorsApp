package com.tautech.cclappoperators.activities.ui_transfer.transfered
...
class TransferedFragment : Fragment(), EMDKManager.EMDKListener, Scanner.StatusListener, Scanner.DataListener {
    ...
    fun searchData(deliveryNumber: Long) {
        ...
        if (viewModel.processedDeliveries.value != null && deliveryNumber > 0) {
            foundDeliveries = viewModel.processedDeliveries.value?.filter { d ->
                d.deliveryNumber == deliveryNumber /*&& d.planificationId == viewModel.planification.value?.id*/
            }
        }
