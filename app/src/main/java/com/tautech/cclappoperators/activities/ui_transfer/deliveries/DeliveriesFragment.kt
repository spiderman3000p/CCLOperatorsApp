package com.tautech.cclappoperators.activities.ui_transfer.deliveries
...
class DeliveriesFragment : Fragment(), EMDKManager.EMDKListener, Scanner.StatusListener, Scanner.DataListener {
    ...
    private val filteredData: MutableList<Delivery> = mutableListOf()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        searchEt4.setOnKeyListener { v, keyCode, event ->
            if (keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_UP) {
                ...
                } else if (!viewModel.deliveries.value.isNullOrEmpty()){
                    filteredData.clear()
        ...
        viewModel.deliveries.observe(viewLifecycleOwner, Observer{ pendingDeliveries ->
            Log.i(TAG, "deliveries observed: $pendingDeliveries")
            if (!pendingDeliveries.isNullOrEmpty()) {
                filteredData.clear()
    ...
    fun searchData(deliveryNumber: Long) {
        ...
        var foundDeliveries = listOf<Delivery>()
